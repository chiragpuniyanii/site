{if $smarty.const.ACCOUNT_TYPE === "vendor"}
    ...
    {$top_bar_height = 56}
    {$top_bar_padding = 11}
    {$image_height = ($top_bar_height - 2 * $top_bar_padding)}

    {$image_data_dark = (!empty($runtime.vendor_panel_style.logo_dark))
        ? ($runtime.vendor_panel_style.logo_dark|fn_image_to_display:$image_height) : "{$images_dir}/cart_logo_white.svg"}
    ...
    {$image_data_light = (!empty($runtime.vendor_panel_style.logo))
        ? ($runtime.vendor_panel_style.logo|fn_image_to_display:$image_height) : "{$images_dir}/cart_logo.svg"}
